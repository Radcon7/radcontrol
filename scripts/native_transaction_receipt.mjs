...
export async function transactionReceiptContext(argv, identities) {
  ...
  assert.ok(['first', 'final'].includes(phase));
  assert.equal(manifest.schemaVersion, 2);
  assert.equal(manifestPath, path.join(manifest.stage.root, 'transaction-manifest.json'));
  assert.equal(manifest.newPair.o2Commit, identities.o2Sha);
  assert.equal(manifest.newPair.radcontrolSourceSha, identities.radcontrolSha);
  assert.equal(manifest.newPair.binarySha256, identities.artifactSha256);
  const state = (await readFile(manifest.stage.stateFile, 'utf8')).trim();
  assert.equal(state, phase === 'first' ? 'new-live' : 'new-live-awaiting-final');
