...
| Native acceptance false isolation | A release artifact can ignore the E2E override and reach installed O2 | A writable test can mutate operator data before post-test cleanliness detects it | Production artifact runs with installed O2 read-only and test-owned `.state`; writable E2E requires canonical preflight plus an in-app exact-root attestation before edits | Installed-root, missing seed, unhonored override, symlink escape, fixture-only persistence, and production before/after integrity tests |
