...
assert.match(app, /const finalUrl = fallbackUrl \?\? urlFromOut/);
assert.match(app, /project\.sourceProjectKey \?\? project\.key/);
assert.match(app, /serviceListening && hostListening/);
assert.match(app, /text: "DEGRADED"/);
assert.match(model, /status === "READY" \|\| status === "RUNNING"/);
...
assert.match(helpers, /sourceProjectKey: optionalStringField\(r, "sourceProjectKey", index\)/);
assert.match(helpers, /logicalSurface: optionalBooleanField\(r, "logicalSurface", index\)/);
...
assert.match(types, /sourceProjectKey\?: ProjectKey/);
assert.match(types, /logicalSurface\?: boolean/);
