  ...
  return (
    <section className="sentinelShell" data-testid="radcon-sentinel">
      <header className={`sentinelHero sentinelThreat-${threat} sentinelOperatorHero`}>
        ...
        <div className="sentinelPrimaryActions" aria-label="Host Guardian actions">
          ...
          <span className="sentinelAutomationControl"><strong>Automatic Guardian · {automaticStatus}</strong>
            ...
            <small>{automationActive ? `Next semantic observation ${formatDateTime(automation?.nextDueAt)}` : automationRequested ? "Timer status unavailable" : "Enable only after an explicit operator decision"}</small>
          </span>
        </div>
      ...
      <section className="sentinelAdvancedWorkspace">
        <header className="sentinelAdvancedHeading"><span>ADVANCED SYSTEM INFORMATION</span><strong>Technical diagnostics, maintenance, automation, governance, and system evidence</strong></header>
        ...
        <section className="sentinelSubCard sentinelDiagnostics" aria-label="Diagnostics">
          <div className="sentinelSubCardHeading"><div><span>DIAGNOSTICS</span><strong>Use deeper deterministic checks when the current evidence needs context</strong></div></div>
          <div className="sentinelActions" aria-label="Sentinel manual read-only checks"><button className="btn btnGhost" type="button" disabled={Boolean(busyAction)} onClick={() => void perform("deep", runHostDeepCheck, "Deep host evidence refreshed.")} data-testid="sentinel-deep-check">{busyAction === "deep" ? "Checking…" : "Deep Check"}</button><button className="btn btnGhost" type="button" disabled={Boolean(busyAction)} onClick={() => void investigateFans()}>{busyAction === "fans" ? "Investigating…" : "Why are my fans running?"}</button></div>
        </section>

        <section className="sentinelGuardianCard" data-testid="host-guardian">
          <div className="sentinelGuardianHeading"><div><span>SENSORS &amp; SYSTEM EVIDENCE</span><strong>Sensor sources, thresholds, processes, services, and provenance</strong></div><StatusPill status={status?.host.overallStatus || "unknown"} /></div>
          <div className="sentinelEvidenceDetails">
            <details className="sentinelDetails"><summary>Thermal sensors, sources, and hardware limits</summary><div className="sentinelCompactList">{thermalRows.map((sensor, index) => <div key={sensor.key || `${sensor.label}-${index}`}><span><strong>{sensor.label || "Unclassified sensor"}{sensor.primary ? " · primary CPU health source" : ""}</strong><small>{sensor.source || "kernel"}{sensor.path ? ` · ${sensor.path}` : ""}</small></span><span>{typeof sensor.temperatureC === "number" ? `${sensor.temperatureC}°C` : "Unknown"}{typeof sensor.criticalC === "number" && sensor.thresholdEligible ? ` · hardware critical ${sensor.criticalC}°C · governed safety margin applied` : " · threshold unavailable; evidence only"}</span></div>)}{!thermalRows.length ? <p>No sensor rows are available. This is not a healthy result.</p> : null}</div></details>
        ...
        <div className="sentinelAuthorityGrid">
          <section className="sentinelSubCard" data-testid="sentinel-capability-ladder"><div className="sentinelSubCardHeading"><div><span>GOVERNANCE &amp; PERMISSIONS</span><strong>What this surface may do</strong></div><small>O2 registry-derived</small></div><div className="sentinelLevelList">{SENTINEL_LEVELS.map((item) => { const levelCapabilities = status?.capabilities.filter((capability) => capability.level === item.level) || []; const levelState = sentinelCapabilityLevelState(item.level, status?.capabilities || []); return <div key={item.level}><span><strong>{item.label}</strong><small>{levelCapabilities.length} declared capabilities</small></span><span className={`sentinelLevelState sentinelLevelState-${levelState}`}>{levelState === "active" ? "ACTIVE · READ ONLY" : "NOT ACTIVATED"}</span></div>; })}</div></section>
          <section className="sentinelSubCard" data-testid="sentinel-triggers"><div className="sentinelSubCardHeading"><div><span>AUTOMATION &amp; SCHEDULES</span><strong>What is active and what is not enabled</strong></div><small>Scheduler {status?.scheduler || "disabled"}</small></div><div className="sentinelTriggerList">{(status?.triggers || []).filter((trigger) => trigger.class === "schedule").map((trigger) => { const lastRun = status ? triggerLastRun(trigger, status) : null; return <div key={trigger.key}><span><strong>{trigger.key === "schedule.light-health" ? "Host Guardian" : trigger.label}</strong><small>{trigger.key === "schedule.light-health" && automationActive ? `Active · ${automation?.frequency === "twice-daily" ? "Twice daily" : "Daily"}` : trigger.activationState === "inactive" ? "Not enabled" : trigger.activationState.replace(/-/g, " ")}</small></span><span>{lastRun ? <small>Last {formatDateTime(lastRun)}</small> : null}{trigger.nextRunAt ? <small>Next {formatDateTime(trigger.nextRunAt)}</small> : null}</span></div>; })}</div></section>
        </div>

        <section className="sentinelAsk"><div><span>QUICK ANSWERS</span><small>Instant deterministic answers from current system data. No AI model used.</small></div><div className="sentinelPromptRow">{ASK_PROMPTS.map((prompt) => <button className="btn btnGhost btnCompact" type="button" key={prompt} onClick={() => setQuestion(prompt)}>{prompt}</button>)}</div><div className="sentinelAskControls"><textarea className="pasteArea" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="Ask about health, visibility, recent activity, or an incident…" /><button className="btn btnPrimary" type="button" onClick={() => void submitQuestion()} disabled={!question.trim() || Boolean(busyAction)}>{busyAction === "ask" ? "Reviewing…" : "Get Quick Answer"}</button></div>{answer ? <div className="sentinelAnswer"><strong>{answer.intent.replace(/-/g, " ")}</strong><p>{answer.answer}</p><small>Execution permitted: NO · AI model used: NO</small></div> : null}</section>

        <details className="sentinelDetails sentinelTechnicalDetails"><summary>Technical Details</summary><div className="sentinelBoundary"><span>Mode: {status?.executionMode || "observe-and-dry-run"}</span><span>Privileged helper: {status?.privilegedHelper || "not-installed"}</span><span>Scheduler: {status?.scheduler || "disabled"}</span><span>Audit: {status?.auditVerification.claim || "hash-chained-not-immutable"}</span></div></details>
      </section>
